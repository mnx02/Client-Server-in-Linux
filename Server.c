...
#include <sys/stat.h>
#include <time.h>
#define PORT 11957 // p0rt for rnning srver
...
printf("Listening to Port 11957----\n");
while (1)
{
//Accpt()
...
MAcmdAccpt++;
printf("Count of Client from server and mirror -->  %d \n", MAcmdAccpt);
if (MAcmdAccpt <= 6)
{
    write(SMsockFileDist2, "Established", strlen("Established"));
}
else if ((MAcmdAccpt >= 7 && MAcmdAccpt <= 12))
{
    
    write(SMsockFileDist2, "MoveToMirror", strlen("MoveToMirror")); // Responded to CLint
}
else if ((MAcmdAccpt > 12 && MAcmdAccpt % 2 == 0))
{
    write(SMsockFileDist2, "MoveToMirror", strlen("MoveToMirror"));
}
else
{
    write(SMsockFileDist2, "Established", strlen("Established"));
